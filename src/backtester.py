...
# Force non-GUI plotting for CI/pytest
if not _is_tty() or os.getenv("PYTEST_CURRENT_TEST"):
    import matplotlib
    matplotlib.use("Agg")
...
# >>> changed import to avoid circulars
from src.engine.runner import run_hedge_fund
...
def _parse_args():
    ...
    p.add_argument("--ticker", default=None, help="Comma-separated tickers override")
    ...
    p.add_argument(
        ...
        type=float,
        default=None,
        help="Margin ratio for short positions, e.g. 0.5 for 50% (overrides config)",
    )
    p.add_argument(
        "--analysts",
        type=str,
    ...
    p.add_argument("--no-interactive", action="store_true", help="Disable interactive prompts (CI/headless)")
    return p.parse_args()
...
class Backtester:
    ...
    def prefetch_data(self):
        if self.headless or os.getenv("OFFLINE") == "1":
            print("Headless/CI mode: skipping data pre-fetch.")
            return
    ...
    def run_backtest(self):
        ...
        for current_date in dates:
            ...
            # Prices — fast path for headless/CI with synthetic panel
            try:
                current_prices = {}
                if self.headless or os.getenv("OFFLINE") == "1":
                    rng = np.random.default_rng(42)
                    if not hasattr(self, "_price_panel"):
                        base = {t: 100.0 + 5.0 * i for i, t in enumerate(self.tickers)}
                        idx = pd.date_range(self.start_date, self.end_date, freq="B")
                        panel = {}
                        for t in self.tickers:
                            steps = rng.normal(0, 0.01, size=len(idx))
                            prices = pd.Series(base[t], index=idx) * (1 + steps).cumprod()
                            panel[t] = prices
                        self._price_panel = pd.DataFrame(panel)
                    prev_dt = pd.to_datetime(previous_date_str)
                    if prev_dt not in self._price_panel.index:
                        continue
                    for t in self.tickers:
                        current_prices[t] = float(self._price_panel.loc[prev_dt, t])
                else:
                    missing_data = False
                    for ticker in self.tickers:
                        try:
    ...
    def _update_performance_metrics(self, performance_metrics):
        ...
        if len(negative_returns) > 0:
            ...
            performance_metrics["sortino_ratio"] = (np.sqrt(252) * (mean_excess_return / downside_std)) if downside_std > 1e-12 else (float("inf") if mean_excess_return > 0 else 0)
        else:
    ...
    def analyze_performance(self):
        ...
        if _is_tty() and not os.getenv("PYTEST_CURRENT_TEST"):
            plt.show()
        else:
            os.makedirs("artifacts", exist_ok=True)
            plt.savefig("artifacts/demo_backtest_equity.png", bbox_inches="tight")
        plt.close()
...
def main():
    ...
    tickers = (args.ticker.split(",") if args.ticker else cfg.get("data.tickers", []))
    start_date = args.start_date or cfg.get("data.start_date") or (datetime.now() - relativedelta(months=1)).strftime("%Y-%m-%d")
    end_date = args.end_date or cfg.get("data.end_date") or datetime.now().strftime("%Y-%m-%d")
    initial_capital = args.initial_capital or cfg.get("backtest.initial_capital", 100000)
    margin_requirement = args.margin_requirement or cfg.get("backtest.margin_requirement", 0.0)

    if not tickers:
        raise SystemExit("No tickers provided (CLI or config).")

    headless = args.no_interactive or (os.getenv("PYTEST_CURRENT_TEST") is not None) or (not _is_tty())
    ...
    if args.analysts_all:
        ...
        print(f"\nSelected analysts: " f"{', '.join(Fore.GREEN + c.title().replace('_', ' ') + Style.RESET_ALL for c in selected_analysts)}")
    ...
    if args.ollama:
        ...
        if headless:
            first = LLM_ORDER[0]
            model_name, model_provider = first[1], first[2]
        else:
    ...
    _ = backtester.analyze_performance()


if __name__ == "__main__":
    # Run as: poetry run python -m src.backtester --no-interactive
    main()
